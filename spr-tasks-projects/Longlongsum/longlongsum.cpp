...
#define LINE_SIZE 4 //data in format "x y\n"
...
struct Input
{
	...
	char* buff; // FIXME dangerous, but fast, reason to refactor solution to collect and solve, rather than collect all and then solve each
	size_t m;
};
		...
		std::function<void(char*, size_t,size_t, size_t)> handle_output)
{
	...
	char buffer[LINE_SIZE * BUFF_SIZE];
	size_t n;
	std::cin >> n;
	std::cin.get();
	for (size_t i = 0; i < n; i++)
	{
		...
		std::cin >> m;
		std::cin.get();
		fread(buffer, LINE_SIZE * m, 1, stdin);
...
static void handle_input(char* buffer, size_t m)
{
	...
	char* line = LINE_SIZE * (m - 1) + buffer;
	int carry = 0;
	for (int j = m - 1; j >= 0; j--)
	{
